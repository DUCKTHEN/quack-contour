...
Quack Contour は、OBJ形式のアバターモデルを読み込み、シルエット、外周線、断面ガイド、上面断面図を通して体型を観察するための実験的なツールです。
...
- 小さめに作られたアバターを、表示上だけ主モデルまたは比較モデルの身長に合わせる。
...
アバターは130cm前後など、小さめのスケールで作られていることがあります。

Quack Contour では、主モデルと比較モデルを表示上だけ同じ身長に合わせて比較できます。

- `主に合わせる`: 比較モデルを主モデルの表示身長に合わせます。
- `比較に合わせる`: 主モデルを比較モデルの表示身長に合わせます。
- `解除`: 表示倍率を元に戻します。

この表示倍率は、画面表示だけでなく、上面図生成や比較図生成の断面寸法にも反映されます。元のOBJファイル自体は変更されません。

## モデルデータについて

読み込んだモデルデータは、モデル制作者、権利者、または利用者に属します。Quack Contour の制作者やリポジトリが、読み込まれたモデルデータに対して権利を主張することはありません。
